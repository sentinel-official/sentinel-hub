...
func NewErrorDuplicateLease(nodeAddr base.NodeAddress, provAddr base.ProvAddress) error {
	return sdkerrors.Wrapf(ErrDuplicateEntry, "lease already exists for node %s by provider %s", nodeAddr, provAddr)
}
...
func NewErrorInvalidProviderStatus(addr base.ProvAddress, status v1base.Status) error {
	return sdkerrors.Wrapf(ErrInvalidStatus, "invalid status %s for provider %s", status, addr)
}
...
func NewErrorLeaseNotFound(id uint64) error {
	return sdkerrors.Wrapf(ErrNotFound, "lease %d does not exist", id)
}
